...
function App() {
  ...
  const [legalHoldReason, setLegalHoldReason] = useState('Investigation hold');
  ...
  async function releaseLegalHold(hold: LegalHold) {
    if (!token || !selectedWorkspaceId) return;
    await api.releaseLegalHold(token, selectedWorkspaceId, hold.id, 'Released from admin console');
    await refreshEnterpriseCompliance();
  }
